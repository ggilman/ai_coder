...
- **`libs/ai-coder-setup.sh`** — the `--setup` wizard (one `setup_step_*` function per step, called in sequence from `cmd_setup`) and `--fix-project` (CRLF→LF normalization for a target project). Sources `ai-coder-gum.sh` itself since `ai-status.sh` and `offline/bundle.sh` source this file directly without going through the full `ai-coder` chain.
- **`libs/ai-coder-gum.sh`** — gum binary bootstrap/resolution (`ensure_gum`, `resolve_gum_cmd`, `_download_gum_binary`). Deliberately self-contained (BASH_SOURCE-relative paths only) since it's sourced standalone by `ai-status.sh` in addition to the full launch chain.
