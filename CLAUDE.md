...
`libs/ai-coder-core.sh` itself sources four sub-libraries (in this order, right after resolving global config/environment): `ai-coder-env.sh` → `ai-coder-settings.sh` → `ai-coder-model.sh` → `ai-coder-workbench.sh`. This split exists purely for file-size readability — treat the five files as one logical unit; none are sourced independently elsewhere, and moving a function between them never needs a change outside `ai-coder-core.sh`'s own source lines.
...
- **`libs/ai-coder-settings.sh`** — git identity (`ensure_git_identity`, `apply_git_identity`, `ensure_container_gitconfig`) and launch-time preference resolution (`ensure_network_config`, `ensure_gpu_config`, `ensure_ctx_config`, `ensure_kv_config`, `ensure_overhead_config`, `ensure_offload_config`).
- **`libs/ai-coder-model.sh`** — Docker preflight (`check_docker`), VRAM/KV budgeting (`_estimate_kv_reserve_gb`, `detect_model`), model tier selection and download (`select_model_for_vram`, `download_model`), and base image pulling (`pull_image_if_missing`, `pull_base_image_via_proxy`).
- **`libs/ai-coder-workbench.sh`** — Dockerfile generation and image builds shared by npm-based agents (`build_standard_image`, `build_npm_agent_image`), the per-project spoke container (`run_workbench`, `exec_in_container`, `ensure_workbench_running`), GPU arg resolution (`_resolve_engine_gpu_args`), fast-storage model volume sync (`ensure_model_in_volume`), the Hub engine itself (`start_hub_engine`), the `--rebuild` image sweep (`rebuild_workbench_images`), the engine restart-decision + (re)start sequence (`ensure_engine_currently_running` — caller must hold the hub-start lock), and the post-start readiness poll (`wait_for_engine_ready`).
