...
- **`libs/ai-coder-env.sh`** — path/shell utilities (`to_host_path`, `merge_json_file`), preference I/O (`read_pref`/`write_pref` against the `key=value` files in `user/`), MCP config JSON generation (`make_agent_mcp_json`), and the update checker (`check_for_update`).
