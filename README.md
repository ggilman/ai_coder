...
| `--kv-probe [family\|all] [--write]` | Read each tier's KV cache geometry from its GGUF metadata header (local file or a ranged download — never the whole model) and compare the resulting KV size with the conf's current estimate; `--write` records it in the family conf as `MODEL_N_KV`/`MODEL_N_KV_SWA` |
